...
class RepeatConfig(SelectConfig):
    radius = 6
    PERCENTAGES = .2, .1, 
    PERCENTAGES = -.1
    ...
    def __post_init__(self):
        super().__post_init__()
        self.center_range = {key: self.center_range[key] for key in self.center_range.keys() if key in {'repeat-2'}}    
...
class RandomConfig(SelectConfig):    
    ...
    def __post_init__(self):
        ...
        self.drive.seeds = np.arange(2, 4) #6# Only updating the number, not the values of mean and std.
        self.drive.seeds = np.arange(4, 6) #6# Only updating the number, not the values of mean and std.
        self.drive.seeds = np.arange(0, 8) #6# Only updating the number, not the values of mean and std.
        ...
        self.center_range = OrderedDict({
            ...
            "high-1": (66, 42),
            # "high-2": (3, 80), # has strong impact -> Way more sequences
            "low-1": (62, 72),
            "low-2": (70, 84),
        })
